...
- 房主神器在大厅即可改生命、伤害、积分、移速和射击间隔；命令实际生效后才提示成功。
- 大厅可以踢出玩家，也可预设下一局胜者；对局内仍支持 AI 接管和强制获胜。
- 玩家停止造成或承受有效伤害 5 秒后，每秒恢复 8 点生命；再次交战立即中断。
- AI 的移动、射程、瞄准、技能使用率和反应速度均已降低。
- 镜头持续以本地人物为中心，复活时立即对准；地图边缘允许扩展视野，不再裁切人物。

## v3.3 改进

- 子弹改为强烈街机风：白色亮核、角色色辉光、方向拖尾、枪口闪光和命中爆裂。
- 拖尾、爆裂和火花使用固定容量对象池；性能降低时只关闭装饰性火花，保留子弹和命中核心反馈。
- 只加入发射、命中/消散、自己受击和技能球拾取四类关键音效，不加入大招或背景音乐。
- 手机首次触摸后解锁音频；大厅和对局右上角都有声音开关，静音状态会保留。
- 远处玩家射击音会按距离衰减并限流，避免六人同时射击时声音混成一团。
