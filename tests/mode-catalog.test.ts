...
describe("v4 match modes", () => {
  it("defines personal, 3v3, and 2v2v2 with scaled team targets", () => {
    ...
    expect(getModeDefinition("solo")).toMatchObject({ teamCount: 0, teamSize: 1, targetScore: 20 });
    expect(getModeDefinition("team3v3")).toMatchObject({ teamCount: 2, teamSize: 3, targetScore: 60 });
    expect(getModeDefinition("team2v2v2")).toMatchObject({ teamCount: 3, teamSize: 2, targetScore: 40 });
  });
